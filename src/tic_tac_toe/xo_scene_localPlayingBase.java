...
public  class xo_scene_localPlayingBase extends AnchorPane {
  ...
  public static ArrayList <Integer> Moves=new ArrayList();  /** Array List to record moves of game */
 
 
   
 public static void initGame()  /** function initialize game by set all board element zero and first player X and state of game Playing */
 {
     for (int row = 0; row < ROWS; ++row) {
         for (int col = 0; col < COLS; ++col) {  /** Looping to initialize the board cells with zero */
            board[row][col] = EMPTY; 
 ...
 public static void updateGame(int theMove, int currentRow, int currentCol) /** check every move if there is winner or draw case */
 {
      if (IsWon(theMove, currentRow, currentCol)) {  /** Update method recall Iswon and Draw function to check */
      ...
      for (int row = 0; row < ROWS; ++row) {
         for (int col = 0; col < COLS; ++col) { 
            if (board[row][col] == EMPTY) {  /** for draw case must all cells be not empty */
               return false;  
            }
         }
      }
 ...
 {
      return (board[currentRow][0] == theMove          /** 3 identical moves in same row */
                   && board[currentRow][1] == theMove
                   && board[currentRow][2] == theMove
              || board[0][currentCol] == theMove      /** 3 identical moves is the same column */
                   && board[1][currentCol] == theMove
                   && board[2][currentCol] == theMove
              || currentRow == currentCol            
                   && board[0][0] == theMove      /** 3 identical moves is diagonal and reversed diagonal */
                   && board[1][1] == theMove   
                   && board[2][2] == theMove
 ...
 public void Game(Button ref ,int x ,int y,int num) /** function called in every button listener to update game each step */
 {
        ...
        if (board[currentRow][currentCol] == EMPTY) /** At first this Cell must be empty */
       {
         switch(currentPlayer)   /** Switch case to put the image of X or O in the board */
        {
             case X: /** The case of Player X */
           ...
           board[currentRow][currentCol] = currentPlayer; /**Put the Value of Current Player in board to be not empty */
            Moves.add(num);  /**Record the move in the array */
           updateGame(currentPlayer, currentRow,currentCol); /** Call Update Method */
           currentPlayer = (currentPlayer == X) ? O : X;  /**Switch Player from X to O and vise versa*/
          }
     } 
 }
